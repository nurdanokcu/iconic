...
<template>
  <header
    ...
    <img :src="headerImage" :alt="modelName" :class="imageClass" />
    <div class="w-full max-content-centered-lg p-4 md:px-6 flex items-end md:items-stretch lg:items-end h-full">
      <div class="flex flex-col justify-between lg:flex-row lg:w-full lg:gap-8">
        <div class="flex flex-col p-4 gap-2 bg-surface-primary max-w-md">
          <h1 class="font-extrabold leading-tight text-2xl uppercase">
            {{ modelName }}
          </h1>
          <div class="flex flex-wrap justify-between gap-x-4">
            <p v-for="tag in modelTags" :key="tag" class="pr-4">
              {{ tag }}
            </p>
          </div>
        </div>
        <ModelsSingleEventTypes class="hidden md:flex" :events="events" />
      </div>
    </div>
